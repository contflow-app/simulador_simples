...
faturamento_projetado = st.number_input("Receita mensal projetada para os próximos meses", value=0.0, step=100.0, format="%.2f")

if receita_bruta_12 and faturamento_atual and faturamento_projetado:
    ...
    # Remover a linha do Mês 0
    df_proj = df_proj[df_proj["Mês"] != "Mês 0"]

    df_proj.set_index("Mês", inplace=True)
    st.markdown("## 📈 Projeção dos Próximos 12 Meses (sem incluir o mês atual)")
    st.table(df_proj)
