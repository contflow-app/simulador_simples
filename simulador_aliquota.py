...
# ---------- AUTENTICAÇÃO ----------
users = {
    "usuario1": {"name": "João Silva", "password": "senha123"},
    "usuario2": {"name": "Maria Souza", "password": "senha456"},
}

hashed_passwords = stauth.Hasher([u["password"] for u in users.values()]).generate()

authenticator = stauth.Authenticate(
    {u: {"name": users[u]["name"], "password": hashed_passwords[i]} for i, u in enumerate(users)},
    "simulador_app",
    "abcdef123456",
