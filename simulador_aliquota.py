...
if st.session_state.receita_acumulada is None:
    ...
    faixa_atual, aliq_efetiva = aliquota_efetiva(st.session_state.receita_acumulada)
    st.info(f"Faixa atual: **{faixa_atual}** | Alíquota efetiva estimada: **{aliq_efetiva*100:.2f}%**")

    # Entrada de faturamento mensal
    faturamento_mes = st.number_input("Digite o faturamento do mês atual:", min_value=0.0, step=100.0)
    ...
    if st.button("Gerar Projeção"):
        ...
        for i in range(6):
            ...
            faixa_proj, aliq_efetiva_proj = aliquota_efetiva(receita_projetada)
